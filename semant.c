...
SEM_ExpType SEM_trans_cond_exp(S_Table venv, S_Table tenv, TR_Exp tr_if_exp,
    ...
    if (exp->u.iff.elsee) {
        ...
        EM_error(exp->u.iff.then->pos, "missing else-clause in if-expression");
    }
        ...
        case A_SIMPLE_VAR:
            {
                ...
                if (entry && entry->kind == E_VAR_ENTRY) {
                    TR_Exp mem = make_TR_MemExp(venv, var->u.simple);
                    TR_TransExp tr = make_TR_TransExp(mem);
                    return make_SEM_ExpType(tr, SEM_actual_type(tenv, entry->u.var.type));
                } else {
        ...
        case A_FIELD_VAR:
            {
                ...
                for (; fields; fields = fields->tail) {
                    if (fields->head->name == var->u.field.sym) {
                        ...
                        TR_Exp var_exp = var_exp_type->exp->u.exp;
                        TR_Exp field_exp = make_TR_FieldExp(var_exp, fields->head->name,
                                T_size(fields->head->type), field_offset);
                        TR_TransExp tr = make_TR_TransExp(field_exp);
                        return make_SEM_ExpType(tr, fields->head->type);
                    }
        ...
        case A_SUBSCRIPT_VAR:
            {
                ...
                assert(var_exp_type->exp && var_exp_type->exp->kind == TR_EXP);
                TR_Exp var_exp = make_TR_VarExp(var_exp_type->exp->u.exp);
                assert(index_exp_type->exp && index_exp_type->exp->kind == TR_EXP);
                int element_size = T_size(var_exp_type->type->u.array);
                TR_Exp subscript_exp = make_TR_SubscriptExp(var_exp, element_size, index_exp_type->exp->u.exp);
                TR_TransExp tr = make_TR_TransExp(subscript_exp);
                return make_SEM_ExpType(tr, var_exp_type->type->u.array); 
            }
    }
    return NULL;
}
...
SEM_ExpType SEM_trans_exp(S_Table venv, S_Table tenv, TR_Function func, A_Exp exp) {
    switch (exp->kind) {
        case A_VAR_EXP:
            {
                ...
                TR_Exp var_exp = make_TR_VarExp(var_exp_type->exp->u.exp);
                TR_TransExp tr = make_TR_TransExp(var_exp);
        ...
        case A_INT_EXP:
            {
                TR_Exp tr_num_exp = make_TR_NumExp(exp->u.intt);
                TR_TransExp tr = make_TR_TransExp(tr_num_exp);
        ...
        case A_CALL_EXP:
            {
                ...
                if (formals) {
                    EM_error(exp->pos, "too few arguments for function %s",
        ...
        case A_ARRAY_EXP:
            {
                ...
                TR_Exp num_elements = NULL;
                if (!size_exp_type->exp || size_exp_type->exp->kind != TR_EXP) {
                    EM_error(exp->u.array.size->pos, "unrecognizable array size");
                } else {
                    num_elements = size_exp_type->exp->u.exp;
                }
                TR_Exp tr_array_exp = make_TR_ArrayExp(num_elements, T_size(element_type), tr_init_exp);
                TR_TransExp tr = make_TR_TransExp(tr_array_exp);
        ...
        case A_SEQ_EXP:
            {
                ...
                TR_Exp tr_seq = make_TR_SeqExp(stms);
                TR_TransExp tr = make_TR_TransExp(tr_seq);
        ...
        case A_FOR_EXP:
            {
                ...
                S_begin_scope(venv);
                TR_add_var(func, exp->u.forr.var, lo_exp_type->type);
                ...
                TR_Stm tr_for_stm = make_TR_ForStm(tr_mem_exp, tr_lo_exp, tr_hi_exp, NULL);
                TR_push_loop(tr_for_stm);
        ...
        case A_BREAK_EXP:
            {
                ...
                TR_Stm tr_break_stm = make_TR_BreakStm();
        ...
        case A_OP_EXP:
            {
                ...
                switch (exp->u.op.oper) {
                    ...
                    case A_TIMES_OP:
                        ...
                        break;
                    case A_DIVIDE_OP:
                        tr_op_exp = make_TR_DivOpExp(left->exp->u.exp, right->exp->u.exp);
                        break;
                    case A_EQ_OP:
                    case A_NEQ_OP:
                    case A_LT_OP:
                    ...
                    case A_GE_OP:
                    case A_GT_OP:
                        tr_op_exp = make_TR_RelOpExp(left->exp->u.exp, right->exp->u.exp, exp->u.op.oper);
                        break;
                    case A_AND_OP:
                    case A_OR_OP:
                        tr_op_exp = make_TR_LogOpExp(left->exp->u.exp, right->exp->u.exp, exp->u.op.oper);
                        break;
                    default:
                        EM_error(exp->pos, "unrecognized operator");
                        break;
                }
                ...
                for (A_FunDecList fdl = dec->u.function; fdl; fdl = fdl->tail) {
                    ...
                    for (
                        ...
                        S_enter(
                                ...
                                make_E_VarEntry(
                                    formals->head,
                                    new_function->frame->nesting_level,
                                    new_function->frame->end
                                    )
                               );
                    }

/*
 * print_ir.h
 * Implementation of facilities for printing the IR representation
...
void P_print_exp(TR_Exp exp, int offset) {
    ...
    switch (exp->kind) {
        ...
        case TR_ARRAY_EXP:
            {
                indent(offset + OFFSET);
                printf("Initializer:\n");
                P_print_exp(exp->u.array.init, offset + 2 * OFFSET);
                break;
            }
        ...
        case TR_DIV_OP_EXP:
            {
                P_print_exp(exp->u.div.left, offset + OFFSET);
                P_print_exp(exp->u.div.right, offset + OFFSET);
                break;
            }
        case TR_REL_OP_EXP:
            {
                indent(offset + OFFSET);
                printf("%s\n", P_op_names[exp->u.arith.op]);
                P_print_exp(exp->u.rel.left, offset + OFFSET);
                P_print_exp(exp->u.rel.right, offset + OFFSET);
                break;
            }
        case TR_LOG_OP_EXP:
            {
                indent(offset + OFFSET);
                printf("%s\n", P_op_names[exp->u.arith.op]);
                P_print_exp(exp->u.log.left, offset + OFFSET);
                P_print_exp(exp->u.log.right, offset + OFFSET);
                break;
            }
        case TR_IF_ELSE_EXP:
            {
                P_print_exp(exp->u.if_else.test, offset + OFFSET);
                indent(offset + OFFSET);
                puts("True:");
                P_print_exp(exp->u.if_else.true_branch, offset + 2 * OFFSET);
                indent(offset + OFFSET);
                puts("False:");
                P_print_exp(exp->u.if_else.false_branch, offset + 2 * OFFSET);
                indent(offset + OFFSET);
                puts("Join:");
                break;
            }
